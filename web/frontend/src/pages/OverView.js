...
export default function OverView() {
    ...
    const [tarmsData, setTarmsData] = useState([]);
    ...
    const transportFeature = () => {
        ...
        for(let index in transport){
            trainList.push([transport[index]["id"], transport[index]["transports_train"]])
            tramList.push([transport[index]["id"], transport[index]["transports_tram"]])
            busList.push([transport[index]["id"], transport[index]["transports_bus"]])
        }
        setTrainsData(trainList)
        setTarmsData(tramList)
        setBusData(busList)
    }
    ...
    const entertainmentFeature = () => {
        ...
        entertainment.sort(sortByProperty('clients_that_recvd_alchl_drug_trtmnt_servs_per_1000_pop'));
        for(let index in entertainment){
            alchlAnddrugsList.push([entertainment[index]["id"], entertainment[index]["clients_that_recvd_alchl_drug_trtmnt_servs_per_1000_pop"]])
        }
    ...
    const reputationFeature = () => {
        ...
        reputation.sort(sortByProperty('ppl_rated_their_cmty_good_vgood_for_cmty_and_sup_grps_perc'));
        for(let index in reputation){
            happinessList.push([reputation[index]["id"], reputation[index]["ppl_rated_their_cmty_good_vgood_for_cmty_and_sup_grps_perc"]])
        }
